...
function Contacts() {
  const mainAddress = locations[0]?.address ?? "";
  return <section id="contacts" className="scroll-mt-20 bg-accent py-section text-accent-foreground"><div className="mx-auto max-w-site px-5 sm:px-8"><SectionHeading eyebrow="Say hello" title="Contacts" /><div className="grid gap-10 border-t border-accent-foreground/25 pt-8 lg:grid-cols-2"><p className="max-w-xl font-display text-3xl leading-snug sm:text-4xl">Questions, collaborations, or just want to say hello? We’d love to hear from you.</p><div className="grid gap-5 text-sm sm:grid-cols-2"><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(mainAddress)}`} target="_blank" rel="noreferrer" className="contact-link"><MapPin />{mainAddress}</a><a href="tel:+37120001234" className="contact-link"><Phone />+371 20 001 234</a><a href="mailto:hello@manulcoffee.lv" className="contact-link"><Mail />hello@manulcoffee.lv</a><div className="flex gap-2">{socials.map(({ label, href, icon: Icon }) => <a key={label} href={href} target="_blank" rel="noreferrer" className="grid size-11 place-items-center rounded-full border border-accent-foreground/30 transition-colors hover:bg-accent-foreground hover:text-accent" aria-label={label}><Icon className="size-4" /></a>)}</div></div></div><p className="mt-12 text-xs text-accent-foreground/70">All contact details and social profiles shown are fictional demo content.</p></div></section>;
}
